...
#define ST7789_LCD_240x320
#define ST7789_LCD_ROTATION 0 // You can adjust the rotation as needed (0, 1, 2, or 3)
...
void st7789_init2(st7789_dev_t *dev, st7789_ll_t *ll)
{
	if (!dev || !ll) {
		return;
	}
...
void st7789_fill_color(st7789_dev_t *dev, uint16_t color)
{
	...
	set_address_window(dev, 0, 0, dev->width - 1, dev->height - 1);
...
{
	if ((x < 0) || (x > dev->width - 1) || (y < 0) || (y > dev->height - 1)) {
		return;
	}
